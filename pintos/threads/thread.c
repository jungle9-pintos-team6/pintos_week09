...
void donate_one(struct thread *giver, struct thread *receiver) {
...
static struct thread *next_thread_to_run(void) {
    if (list_empty(&ready_list)) {
        ...
        // priority_less가 높은 우선순위를 앞에 두므로 첫 번째 요소 가져오기
...
void thread_sleep(int64_t wakeup_time) {
    ...
    list_insert_ordered(&sleep_list, &cur->sleep_elem, wakeup_time_less, NULL);
    thread_block();
    intr_set_level(old_level);
}
